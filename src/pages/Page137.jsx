...
// ───────────────────────────────────────────────────────────
// 美容師・感動カウンセリング (App.jsx) - δ方式 v1.0
...
const QUESTIONS = [
  {
    id: "q1",
    title: "ニーズの場面",
    q: "「美容師・感動カウンセリング」が必要となる、最近の具体的シーンを教えてください。",
    placeholder: "例: 先週、〇〇の場面で〇〇しようとして..."
  },
  {
    id: "q2",
    title: "過去の経験",
    q: "これまで「客が気づいていない魅力」に関連する印象的な経験は？",
    placeholder: "例: 〇〇の場面で〇〇した結果..."
  },
  {
    id: "q3",
    title: "理想の機能",
    q: "「美容師・感動カウンセリング」が完璧に機能した状態を描写してください。",
    placeholder: "例: 〇〇が解決し、〇〇という結果が..."
  },
  {
    id: "q4",
    title: "障害",
    q: "「美容師・感動カウンセリング」を妨げている要因は何ですか？",
    placeholder: "例: 知識不足、時間不足、相手側の理解..."
  },
  {
    id: "q5",
    title: "最初の一歩",
    q: "今すぐ・今週試せる、「美容師・感動カウンセリング」の小さな一歩は？",
    placeholder: "例: 〇〇について調べる、〇〇に相談する..."
  }
];

const HISTORY_KEY = "app137_history_v1";
...
const buildAnalysisPrompt = (userName, answers) => {
  ...
  return `あなたは「美容師・感動カウンセリングの専門家」です。以下のユーザーの回答から、客が気づいていない魅力ための深い分析を提供してください。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "14px 18px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: C.gold }}>美容師・感動カウンセリング</div>
            <div style={{ fontSize: 10, color: C.textMuted }}>客が気づいていない魅力</div>
          </div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "28px 18px 40px" }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 12, lineHeight: 1.5 }}>美容師・感動カウンセリング</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>客が気づいていない魅力。あなたの中に眠る答えを引き出します。</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "18px 18px 40px" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={async () => {
              const text = `【美容師・感動カウンセリング】${selectedHistory.date}\n\n${selectedHistory.analysis}`;
