...
const RALLY_QUESTIONS = [
  { id: "q1", step: 1, q: "今、「海外」について頭の中にあることを全部書き出してください。", placeholder: "例：海外が気がかり、最近海外について思い出すこと、引っかかっている言葉... 小さなことでも全部" },
  ...
  { id: "q4", step: 4, q: "これまでに海外についてどう取り組んできましたか？効いた・効かなかったのは？", placeholder: "例：本を読んだ / 誰かに相談した / 試したが続かなかった など" },
  { id: "q5", step: 5, q: "もし「海外」が理想の状態に近づいたら、何が一番変わると思いますか？", placeholder: "例：朝の気分が変わる / 周りとの関係が変わる / 自信がつく など" },
  { id: "q6", step: 6, q: "「海外」のうち、自分でコントロールできる部分はどこですか？できない部分は？", placeholder: "コントロールできる：...\nコントロールできない：..." },
  ...
  { id: "q9", step: 9, q: "1年後、「海外」はどうなっていそうですか？残っているとしたら何が変わっていますか？", placeholder: "例：解決している / 形を変えて残っている / 別の課題に置き換わる など" },
...
時間スコープ: ${(persona.time || []).join("・") || "(未選択)"}`;
  ...
  return `あなたは「「海外向けPR」の専門家」です。認知行動療法(CBT)・成功循環モデル・マズロー欲求段階の知見を踏まえ、以下の人物の海外を構造化し、解放への道筋を提示してください。
...
### 【本当の原因の特定】
表面的な海外の奥にある根本原因を、ペルソナ4軸と10ラリー回答を統合して論理的に示してください（300〜400字）。
...
const buildPerspectivePrompt = (userName, analysisText) => {
  return `あなたは「他者視点分析の専門家」です。以下の海外向けPR分析を踏まえ、3つの異なる視点からその人を見たときに何が見えるかを描写してください。
...
const buildDeepDivePrompt = (userName, analysisText, theme) => {
  return `あなたは「強み言語化の専門家」です。以下の人物の海外向けPR分析結果を踏まえ、特定のテーマについて深掘りした追加分析を行ってください。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "12px 16px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, boxShadow: "0 1px 6px rgba(0,0,0,0.06)", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: C.goldDim }}>海外向けPR</div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "24px 18px 40px", animation: "fadeUp 0.4s ease" }}>
          <div style={{ textAlign: "center", marginBottom: 24 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 8, lineHeight: 1.5 }}>海外向けPR v2</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>「海外向けPR」を<span style={{ color: C.goldDim, fontWeight: 600 }}>4階層カテゴリ・10問ラリー・6軸レーダー</span>で構造化し、自分視点で深掘りする。CBT × 成功循環 × マズロー。</div>
          </div>
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: 18, marginBottom: 20 }}>
            {[
              ...
              { n: "02", t: "海外向けPRレーダー", d: "睡眠・食欲・集中・気分・対人・興味の6軸を1〜5で評価" },
      ...
      {screen === "persona" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 32px" }}>
          ...
          <button onClick={personaReady ? goToRadar : null} disabled={!personaReady}
            ...
            {personaReady ? "▶ 海外向けPRレーダーへ" : "まずは1項目以上選択"}
          </button>
      ...
      {screen === "result" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 32px" }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: C.goldDim, marginBottom: 12 }}>📋 海外向けPR分析</div>
