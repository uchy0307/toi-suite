...
const QUESTIONS = [
  {
    id: "q1",
    title: "時間を忘れた経験",
    q: "時間を忘れて取り組んだことは何ですか？\n過去〜現在で思い浮かぶものを全て書いてください。",
    placeholder: "例：〇〇している時は気づいたら何時間も経っていた..."
  },
  {
    id: "q2",
    title: "動けない時との違い",
    q: "「やる気がでない」時と「燃える」時を比べて、何が違うと思いますか？\n状況・環境・人・タスクの種類——何でも。",
    placeholder: "例：やる気がない時は〇〇、燃える時は〇〇..."
  },
  {
    id: "q3",
    title: "エネルギーの源",
    q: "承認・達成・貢献・成長・楽しみ・刺激——あなたが最もエネルギーになるのはどれですか？\nなぜそう感じるか教えてください。",
    placeholder: "例：〇〇が一番エネルギーになる。なぜなら..."
  },
  {
    id: "q4",
    title: "過去最高のモチベーション",
    q: "過去で最もモチベーションが高かった時期はいつですか？\nその時、何がそうさせていたと思いますか？",
    placeholder: "例：〇〇の時期が一番モチベが高かった。その理由は..."
  },
  {
    id: "q5",
    title: "頼まれなくてもやること",
    q: "誰かに頼まれなくても自然にやっていること、気づいたらやっていることは何ですか？",
    placeholder: "例：頼まれなくても〇〇してしまう..."
  }
];

const HISTORY_KEY = "app08_history_v1";
...
const buildAnalysisPrompt = (userName, answers) => {
  ...
  return `あなたは「モチベーション源泉分析の専門家」です。以下のユーザーの回答を踏まえ、深く分析してください。

あなたは「モチベーション分析の専門家」です。

### 【あなたのモチベーション源泉マップ】
何がエネルギーを生むのか、5つのキーワードで整理する。

### 【ゾーンに入る5条件】
最高のパフォーマンスが出る状態を生む具体的な条件を5つ挙げる。

### 【エネルギーを奪う状況の特定】
モチベを下げる条件・環境を明確にし対処法を示す。

### 【理想の環境設計】
モチベが最大化する働き方・生活スタイルを提案する。

### 【今週の実験】
1つの条件を意識的に整えて試すアクションを提案する。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "14px 18px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: C.gold }}>モチベーション源泉分析</div>
            <div style={{ fontSize: 10, color: C.textMuted }}>あなたを動かしている本当の燃料を発見</div>
          </div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "28px 18px 40px" }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 12, lineHeight: 1.5 }}>モチベーション源泉分析</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>あなたを動かしている本当の燃料を発見。あなたの中に眠る答えを引き出します。</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "18px 18px 40px" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={async () => {
              const text = `【モチベーション源泉分析】${selectedHistory.date}\n\n${selectedHistory.analysis}`;
