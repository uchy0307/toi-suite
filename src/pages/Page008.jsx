// === Page008.jsx === モチベーション源泉分析 - δ方式 v1.0
// Auto-generated from Page004 template (Phase 1 mass deploy)
// Theme keywords: モチベーション, 源泉, 分析

import React, { useState, useRef, useEffect } from "react";

// ───────────────────────────────────────────────────────────
// モチベーション源泉分析 v2.0 - 4階層カテゴリ＋10問ラリー＋6軸レーダー (Page008)
// δ方式実装: API呼び出しゼロ、プロンプト生成のみ
// ───────────────────────────────────────────────────────────
// テーマ: モチベーション源泉分析 を構造化し、本人視点での再認識と次の一手を引き出す
// 構造:
//  - 4階層カテゴリ: 生活領域 / 感情タイプ / 思考パターン / 時間スコープ
//  - 10回会話ラリー: Q1〜Q10の段階的ヒアリング、進捗バー、前へ/次へ/スキップ
//  - モチベーション源泉分析レーダー: 6軸 (自覚/達成/貢献/自由/承認/循環)
...
const RALLY_QUESTIONS = [
  { id: "q1", step: 1, q: "直近で「よし、やるぞ」と思った瞬間は、いつ・どんな場面でしたか？", placeholder: "例：先週の月曜朝 / プロジェクトのキックオフで など。具体的な場面を1つ" },
  { id: "q2", step: 2, q: "その時に動いた源泉は「達成」「貢献」「自由」「承認」のどれが強かった？", placeholder: "1番強かったものと、その理由を一言で" },
  { id: "q3", step: 3, q: "逆に最近モチベーションが低かった理由を、源泉が満たされていない視点で", placeholder: "例：達成感がない / 誰の役にも立っていない感覚 / 縛られている / 認められていない など" },
  { id: "q4", step: 4, q: "あなたの主源泉（最も強い）と副源泉（次に強い）を一つずつ挙げると？", placeholder: "主源泉：____\n副源泉：____\n（達成・貢献・自由・承認・成長・つながりから）" },
  { id: "q5", step: 5, q: "その主源泉が満たされる場面を、今の生活から3つ思い出してください", placeholder: "仕事・家庭・趣味のどこで満たされているか、具体的に" },
  { id: "q6", step: 6, q: "その主源泉を今の環境で最大限活かすには、何をすべきですか？", placeholder: "例：週1で〇〇の場を作る / ◯◯を任せてもらう など" },
  { id: "q7", step: 7, q: "逆に、源泉を消耗させている「やりすぎ・無理」は何ですか？", placeholder: "例：会議が多すぎ / 雑用に時間を取られすぎ / 1人で抱え込みすぎ など" },
  { id: "q8", step: 8, q: "モチベーションが循環するなら、今は「上り・下り・底・平地」のどれ？", placeholder: "現在地と、その理由を一言で" },
  { id: "q9", step: 9, q: "「無理に上げない」前提で、今月の自分とどう付き合いたい？", placeholder: "上りなら加速 / 下りなら回復 / 底なら待つ / 平地なら整える、など" },
  { id: "q10", step: 10, q: "今夜、源泉を1滴満たすためにできる、小さな行動は？", placeholder: "例：3行日記 / 感謝のLINE1通 / 5分の散歩 など、3分以内でできること" },
];
...
const HEALTH_AXES = [
  { id: "clar", icon: "🧭", label: "自覚", desc: "自分の源泉を言語化できる度合い" },
  { id: "ener", icon: "🏆", label: "達成", desc: "達成感が満たされている度合い" },
  { id: "conn", icon: "🫶", label: "貢献", desc: "誰かの役に立てている感覚" },
  { id: "grow", icon: "🪽", label: "自由", desc: "自分の裁量で動けている度合い" },
  { id: "joy", icon: "🌟", label: "承認", desc: "認められている実感" },
  { id: "peac", icon: "🔁", label: "循環", desc: "上下を受け入れられる度合い" },
];
...
時間スコープ: ${(persona.time || []).join("・") || "(未選択)"}`;
  ...
  return `あなたは「モチベーション源泉分析の専門家」です。認知行動療法(CBT)・成功循環モデル・マズロー欲求段階の知見を踏まえ、以下の人物のモチベーションを構造化し、解放への道筋を提示してください。
...
### 【本当の原因の特定】
表面的なモチベーションの奥にある根本原因を、ペルソナ4軸と10ラリー回答を統合して論理的に示してください（300〜400字）。
...
const buildPerspectivePrompt = (userName, analysisText) => {
  return `あなたは「他者視点分析の専門家」です。以下のモチベーション源泉分析分析を踏まえ、3つの異なる視点からその人を見たときに何が見えるかを描写してください。
...
const buildDeepDivePrompt = (userName, analysisText, theme) => {
  return `あなたは「強み言語化の専門家」です。以下の人物のモチベーション源泉分析分析結果を踏まえ、特定のテーマについて深掘りした追加分析を行ってください。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "12px 16px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, boxShadow: "0 1px 6px rgba(0,0,0,0.06)", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: C.goldDim }}>モチベーション源泉分析</div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "24px 18px 40px", animation: "fadeUp 0.4s ease" }}>
          <div style={{ textAlign: "center", marginBottom: 24 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 8, lineHeight: 1.5 }}>モチベーション源泉分析 v2</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>「モチベーション源泉分析」を<span style={{ color: C.goldDim, fontWeight: 600 }}>4階層カテゴリ・10問ラリー・6軸レーダー</span>で構造化し、自分視点で深掘りする。CBT × 成功循環 × マズロー。</div>
          </div>
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: 18, marginBottom: 20 }}>
            {[
              ...
              { n: "02", t: "モチベーション源泉分析レーダー", d: "睡眠・食欲・集中・気分・対人・興味の6軸を1〜5で評価" },
      ...
      {screen === "persona" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 32px" }}>
          ...
          <button onClick={personaReady ? goToRadar : null} disabled={!personaReady}
            ...
            {personaReady ? "▶ モチベーション源泉分析レーダーへ" : "まずは1項目以上選択"}
          </button>
      ...
      {screen === "result" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 32px" }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: C.goldDim, marginBottom: 12 }}>📋 モチベーション源泉分析分析</div>
