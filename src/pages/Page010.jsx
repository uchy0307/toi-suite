...
const QUESTIONS = [
  {
    id: "q1",
    title: "今の1日の問題点",
    q: "今の1日のスケジュールで「いらない時間」「なくなれば良いと感じる時間」は何ですか？",
    placeholder: "例：〇〇の時間が無駄だと感じる..."
  },
  {
    id: "q2",
    title: "理想の朝",
    q: "理想の朝は何時起き？
朝一番にしたいことは何ですか？起床から2時間の理想を教えてください。",
    placeholder: "例：〇時に起きて、まず〇〇をして、その後〇〇..."
  },
  {
    id: "q3",
    title: "仕事の理想配分",
    q: "仕事時間を「創造・管理・コミュニケーション」に分けるとしたら、理想の割合は何%ずつですか？",
    placeholder: "例：創造60%、管理20%、コミュニケーション20%..."
  },
  {
    id: "q4",
    title: "絶対に入れたい習慣",
    q: "1日の中で「これがあると最高」という習慣・行動・時間はありますか？",
    placeholder: "例：〇〇（運動/読書/瞑想等）を〇〇時に入れたい..."
  },
  {
    id: "q5",
    title: "理想の夜",
    q: "夜、どんな状態で眠りにつきたいですか？
そのために夜の時間をどう使いたいですか？",
    placeholder: "例：〇〇という気持ちで眠りにつきたい..."
  }
];

const HISTORY_KEY = "app10_history_v1";
...
const buildAnalysisPrompt = (userName, answers, mode = "deep") => {
  ...
  return `あなたは「理想の1日設計の専門家」です。以下のユーザーの回答を踏まえ、深く分析してください。

あなたは「時間設計の専門家」です。

### 【理想の1日タイムライン】
起床から就寝まで具体的な時間と行動を時系列で示す。

### 【現状とのギャップ分析】
理想と現実の最も大きなギャップを3つ特定する。

### 【明日から変えられること】
ギャップを埋めるために明日から変えられる最小の習慣を提示する。

### 【週間設計のポイント】
1日ではなく週単位で理想を実現するためのリズムを提案する。

### 【エネルギー管理の鍵】
この人の特性に合ったエネルギー管理の方法を提案する。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "14px 18px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: C.gold }}>理想の1日設計</div>
            <div style={{ fontSize: 10, color: C.textMuted }}>理想の1日をデザインし、人生の質を上げる</div>
          </div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "28px 18px 40px" }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 12, lineHeight: 1.5 }}>理想の1日設計</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>理想の1日をデザインし、人生の質を上げる。あなたの中に眠る答えを引き出します。</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "18px 18px 40px" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={async () => {
              const text = `【理想の1日設計】${selectedHistory.date}\n\n${selectedHistory.analysis}`;
