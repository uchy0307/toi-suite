// === Page010.jsx === 理想の1日設計 - δ方式 v1.0
// Auto-generated from Page004 template (Phase 1 mass deploy)
// Theme keywords: 理想, 1日, 設計

import React, { useState, useRef, useEffect } from "react";

// ───────────────────────────────────────────────────────────
// 理想の1日設計 v2.0 - 4階層カテゴリ＋10問ラリー＋6軸レーダー (Page010)
// δ方式実装: API呼び出しゼロ、プロンプト生成のみ
// ───────────────────────────────────────────────────────────
// テーマ: 理想の1日設計 を構造化し、本人視点での再認識と次の一手を引き出す
// 構造:
//  - 4階層カテゴリ: 生活領域 / 感情タイプ / 思考パターン / 時間スコープ
//  - 10回会話ラリー: Q1〜Q10の段階的ヒアリング、進捗バー、前へ/次へ/スキップ
//  - 理想の1日設計レーダー: 6軸 (自分時間/ゴール/境界線/優先/儀式/満足)
...
const RALLY_QUESTIONS = [
  { id: "q1", step: 1, q: "直近の1日で、「自分のための時間」は何分ありましたか？", placeholder: "厳密でなくてOK。0分なら0分と。読書・散歩・コーヒーなど、家族や仕事以外の自分時間" },
  { id: "q2", step: 2, q: "その時間に何をしましたか？10点満点でつけるなら満足度は？", placeholder: "やったこと：____\n満足度：__/10\n理由を一言で" },
  { id: "q3", step: 3, q: "50歳の自分が後悔しないために、毎日30分どこに組み込みたいですか？", placeholder: "例：朝の運動 / 夜の読書 / 配偶者との会話 / 1人の静寂 など" },
  { id: "q4", step: 4, q: "「家族・部下のため」と「自分のため」の境界線は、いま何時にありますか？", placeholder: "現状の境界線と、理想の境界線をそれぞれ" },
  { id: "q5", step: 5, q: "朝起きた瞬間に「今日のゴール3つ」を即答できますか？できないなら何が邪魔？", placeholder: "即答できる/できない の理由を具体的に" },
  { id: "q6", step: 6, q: "理想の1日が描けないのは、時間の問題ですか？優先順位の決断の問題ですか？", placeholder: "両方なら、より大きい方を一言で" },
  { id: "q7", step: 7, q: "理想の1日に絶対外せない儀式は、朝・昼・夜それぞれ何ですか？", placeholder: "朝：____\n昼：____\n夜：____\n（1つずつ、3分でもOK）" },
  { id: "q8", step: 8, q: "その儀式を1週間続けると、何が変わりそうですか？", placeholder: "気分？体調？家族との関係？仕事のパフォーマンス？" },
  { id: "q9", step: 9, q: "実行を阻む最大の障害は何ですか？（人？環境？習慣？）", placeholder: "障害を1つに絞って書き出すと、対策が見えやすくなります" },
  { id: "q10", step: 10, q: "明日、その障害を回避して儀式を1つ実行する小さな一歩は？", placeholder: "例：5分早く起きる / スマホを別室に置く / 配偶者に宣言する など" },
];
...
const HEALTH_AXES = [
  { id: "clar", icon: "🕰️", label: "自分時間", desc: "1日の中で確保できている度合い" },
  { id: "ener", icon: "🎯", label: "ゴール", desc: "朝にゴールが明確な度合い" },
  { id: "conn", icon: "🚪", label: "境界線", desc: "家族・仕事との境界が整っている度合い" },
  { id: "grow", icon: "⚖️", label: "優先", desc: "やる/やらないを即決できる度合い" },
  { id: "joy", icon: "🌅", label: "儀式", desc: "続けられる習慣の充実" },
  { id: "peac", icon: "😌", label: "満足", desc: "1日終わりの満足感" },
];
...
時間スコープ: ${(persona.time || []).join("・") || "(未選択)"}`;
  ...
  return `あなたは「理想の1日設計の専門コーチ」です。認知行動療法(CBT)・成功循環モデル・マズロー欲求段階の知見を踏まえ、以下の人物の理想を構造化し、解放への道筋を提示してください。
...
### 【本当の原因の特定】
表面的な理想の奥にある根本原因を、ペルソナ4軸と10ラリー回答を統合して論理的に示してください（300〜400字）。
...
const buildPerspectivePrompt = (userName, analysisText) => {
  return `あなたは「他者視点分析の専門家」です。以下の理想の1日設計分析を踏まえ、3つの異なる視点からその人を見たときに何が見えるかを描写してください。
...
const buildDeepDivePrompt = (userName, analysisText, theme) => {
  return `あなたは「強み言語化の専門家」です。以下の人物の理想の1日設計分析結果を踏まえ、特定のテーマについて深掘りした追加分析を行ってください。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "12px 16px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, boxShadow: "0 1px 6px rgba(0,0,0,0.06)", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: C.goldDim }}>理想の1日設計</div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "24px 18px 40px", animation: "fadeUp 0.4s ease" }}>
          <div style={{ textAlign: "center", marginBottom: 24 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 8, lineHeight: 1.5 }}>理想の1日設計 v2</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>「理想の1日設計」を<span style={{ color: C.goldDim, fontWeight: 600 }}>4階層カテゴリ・10問ラリー・6軸レーダー</span>で構造化し、自分視点で深掘りする。CBT × 成功循環 × マズロー。</div>
          </div>
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: 18, marginBottom: 20 }}>
            {[
              ...
              { n: "02", t: "理想の1日設計レーダー", d: "睡眠・食欲・集中・気分・対人・興味の6軸を1〜5で評価" },
      ...
      {screen === "persona" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 32px" }}>
          ...
          <button onClick={personaReady ? goToRadar : null} disabled={!personaReady}
            ...
            {personaReady ? "▶ 理想の1日設計レーダーへ" : "まずは1項目以上選択"}
          </button>
      ...
      {screen === "result" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 32px" }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: C.goldDim, marginBottom: 12 }}>📋 理想の1日設計分析</div>
