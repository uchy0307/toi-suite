// === Page139.jsx === 高齢者レク・脳トレ企画 - δ方式 v1.0
// Auto-generated from Page004 template (Phase 1 mass deploy)
// Theme keywords: 高齢者, レク, 脳トレ

import React, { useState, useRef, useEffect } from "react";

// ───────────────────────────────────────────────────────────
// 高齢者レク・脳トレ企画 v2.0 - 4階層カテゴリ＋10問ラリー＋6軸レーダー (Page139)
// δ方式実装: API呼び出しゼロ、プロンプト生成のみ
// ───────────────────────────────────────────────────────────
// テーマ: 高齢者レク・脳トレ企画 を構造化し、本人視点での再認識と次の一手を引き出す
// 構造:
//  - 4階層カテゴリ: 生活領域 / 感情タイプ / 思考パターン / 時間スコープ
//  - 10回会話ラリー: Q1〜Q10の段階的ヒアリング、進捗バー、前へ/次へ/スキップ
//  - 高齢者レク・脳トレ企画レーダー: 6軸 (睡眠/食欲/集中/気分/対人/興味)
...
時間スコープ: ${(persona.time || []).join("・") || "(未選択)"}`;
  ...
  return `あなたは「「高齢者レク・脳トレ企画」の専門家」です。認知行動療法(CBT)・成功循環モデル・マズロー欲求段階の知見を踏まえ、以下の人物の高齢者を構造化し、解放への道筋を提示してください。
...
const buildPerspectivePrompt = (userName, analysisText) => {
  return `あなたは「他者視点分析の専門家」です。以下の高齢者レク・脳トレ企画分析を踏まえ、3つの異なる視点からその人を見たときに何が見えるかを描写してください。
...
const buildDeepDivePrompt = (userName, analysisText, theme) => {
  return `あなたは「強み言語化の専門家」です。以下の人物の高齢者レク・脳トレ企画分析結果を踏まえ、特定のテーマについて深掘りした追加分析を行ってください。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "12px 16px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, boxShadow: "0 1px 6px rgba(0,0,0,0.06)", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: C.goldDim }}>高齢者レク・脳トレ企画</div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "24px 18px 40px", animation: "fadeUp 0.4s ease" }}>
          <div style={{ textAlign: "center", marginBottom: 24 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 8, lineHeight: 1.5 }}>高齢者レク・脳トレ企画 v2</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>「高齢者レク・脳トレ企画」を<span style={{ color: C.goldDim, fontWeight: 600 }}>4階層カテゴリ・10問ラリー・6軸レーダー</span>で構造化し、自分視点で深掘りする。CBT × 成功循環 × マズロー。</div>
          </div>
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: 18, marginBottom: 20 }}>
            {[
              ...
              { n: "02", t: "高齢者レク・脳トレ企画レーダー", d: "睡眠・食欲・集中・気分・対人・興味の6軸を1〜5で評価" },
      ...
      {screen === "persona" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 32px" }}>
          ...
          <button onClick={personaReady ? goToRadar : null} disabled={!personaReady}
            ...
            {personaReady ? "▶ 高齢者レク・脳トレ企画レーダーへ" : "まずは1項目以上選択"}
          </button>
      ...
      {screen === "result" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 32px" }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: C.goldDim, marginBottom: 12 }}>📋 高齢者レク・脳トレ企画分析</div>
