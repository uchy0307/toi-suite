// === Page007.jsx === コンプレックス反転AI - δ方式 v1.0
// Auto-generated from Page004 template (Phase 1 mass deploy)
// Theme keywords: コンプレックス, 反転, 強み

import React, { useState, useRef, useEffect } from "react";

// ──────────────────────────────────────────────────────────────
// コンプレックス反転AI v2.0 - 4階層カテゴリ＋10問ラリー＋6軸レーダー (Page007)
// δ方式実装: API呼び出しゼロ、プロンプト生成のみ
// ────────────────────────────────────────────────────────────
// テーマ: コンプレックス反転AI を構造化し、本人視点での再認識と次の一手を引き出す
// 構造:
//  - 4階層カテゴリ: 生活領域 / 感情タイプ / 思考パターン / 時間スコープ
//  - 10回会話ラリー: Q1〜Q10の段階的ヒアリング、進捗バー、前へ/次へ/スキップ
//  - コンプレックス反転AIレーダー: 6軸 (自覚/工夫/共感/反転/受容/平穏)
...
const RALLY_QUESTIONS = [
  { id: "q1", step: 1, q: "子どもの頃から自分を悩ませてきたコンプレックスは何ですか？", placeholder: "例：身長 / 話し方 / 容姿 / 学歴 / 家庭環境 など。複数あってもOK" },
  { id: "q2", step: 2, q: "そのコンプレックスを最初に意識したのは、いつ・どんな場面でしたか？", placeholder: "例：小学校で〇〇と言われた / 中学で気づいた など、具体的な出来事を1つ" },
  { id: "q3", step: 3, q: "そのコンプレックスがあったから身につけた「工夫・スキル・習慣」は？", placeholder: "例：話下手だからメモを取る癖 / 容姿のために身だしなみを徹底 など" },
  { id: "q4", step: 4, q: "その工夫を始めて、結果として何が変わりましたか？", placeholder: "例：仕事で評価された / 安心感が増えた / 別のスキルが身についた など" },
  { id: "q5", step: 5, q: "同じコンプレックスを持つ他人がいたら、何とアドバイスしますか？", placeholder: "あなた自身が経験者として伝えられる言葉を1つ" },
  { id: "q6", step: 6, q: "そのコンプレックスをそのまま「強み」として言語化するなら、どんな見出しに？", placeholder: "例：「人見知りだから観察力が育った」「不器用だから工夫の引き出しが増えた」など" },
  { id: "q7", step: 7, q: "工夫のプロセスを学べば、他にどんな課題に応用できそうですか？", placeholder: "例：人材育成 / 自社サービス改善 / 子育て など、波及先を3つ" },
  { id: "q8", step: 8, q: "コンプレックスを「無くす」のではなく「持ったまま生きる」なら、どう見える？", placeholder: "それを持ったままの自分が、5年後にどんな表情をしているか" },
  { id: "q9", step: 9, q: "そのコンプレックスのおかげで救われた人がいるとしたら、誰でしたか？", placeholder: "あなたの工夫や視点に助けられた、家族・同僚・後輩などを思い出して" },
  { id: "q10", step: 10, q: "今夜できる、コンプレックス反転の小さな一歩は？", placeholder: "例：誰かに自分の工夫を話す / 日記に1行書く / SNSに小さく書く など" },
];
...
const HEALTH_AXES = [
  { id: "clar", icon: "🪞", label: "自覚", desc: "コンプレックスを言語化できる度合い" },
  { id: "ener", icon: "🛠️", label: "工夫", desc: "そこから生まれた習慣・スキルの数" },
  { id: "conn", icon: "🫂", label: "共感", desc: "同じ痛みを持つ他者への共感力" },
  { id: "grow", icon: "🔄", label: "反転", desc: "強みへの変換が進んでいる度合い" },
  { id: "joy", icon: "😊", label: "受容", desc: "コンプレックスを抱えたまま笑える度合い" },
  { id: "peac", icon: "🕊️", label: "平穏", desc: "コンプレックスに振り回されない度合い" },
];
...
時間スコープ: ${(persona.time || []).join("・") || "(未選択)"}`;
  ...
  return `あなたは「「コンプレックス反転AI」の専門家」です。認知行動療法(CBT)・成功循環モデル・マズロー欲求段階の知見を踏まえ、以下の人物のコンプレックスを構造化し、解放への道筋を提示してください。
...
### 【本当の原因の特定】
表面的なコンプレックスの奥にある根本原因を、ペルソナ4軸と10ラリー回答を統合して論理的に示してください（300〜400字）。
...
const buildPerspectivePrompt = (userName, analysisText) => {
  return `あなたは「他者視点分析の専門家」です。以下のコンプレックス反転AI分析を踏まえ、3つの異なる視点からその人を見たときに何が見えるかを描写してください。
...
const buildDeepDivePrompt = (userName, analysisText, theme) => {
  return `あなたは「強み言語化の専門家」です。以下の人物のコンプレックス反転AI分析結果を踏まえ、特定のテーマについて深掘りした追加分析を行ってください。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "12px 16px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, boxShadow: "0 1px 6px rgba(0,0,0,0.06)", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: C.goldDim }}>コンプレックス反転AI</div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "24px 18px 40px", animation: "fadeUp 0.4s ease" }}>
          <div style={{ textAlign: "center", marginBottom: 24 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 8, lineHeight: 1.5 }}>コンプレックス反転AI v2</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>「コンプレックス反転AI」を<span style={{ color: C.goldDim, fontWeight: 600 }}>4階層カテゴリ・10問ラリー・6軸レーダー</span>で構造化し、自分視点で深掘りする。CBT × 成功循環 × マズロー。</div>
          </div>
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: 18, marginBottom: 20 }}>
            {[
              ...
              { n: "02", t: "コンプレックス反転AIレーダー", d: "睡眠・食欲・集中・気分・対人・興味の6軸を1〜5で評価" },
      ...
      {screen === "persona" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 32px" }}>
          ...
          <button onClick={personaReady ? goToRadar : null} disabled={!personaReady}
            ...
            {personaReady ? "▶ コンプレックス反転AIレーダーへ" : "まずは1項目以上選択"}
          </button>
      ...
      {screen === "result" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 32px" }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: C.goldDim, marginBottom: 12 }}>📋 コンプレックス反転AI分析</div>
