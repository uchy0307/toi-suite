...
const QUESTIONS = [
  {
    id: "q1",
    title: "コンプレックスの正体",
    q: "自分が一番コンプレックスに感じていることは何ですか？\nできる限り正直に、具体的に教えてください。",
    placeholder: "例：〇〇が人より劣っていると感じている..."
  },
  {
    id: "q2",
    title: "いつから",
    q: "そのコンプレックスはいつ頃から意識するようになりましたか？\nきっかけになった出来事はありますか？",
    placeholder: "例：〇〇の時に〇〇と言われてから..."
  },
  {
    id: "q3",
    title: "気づかせてくれたもの",
    q: "そのコンプレックスがあったから「気づけたこと」「学べたこと」は何かありますか？",
    placeholder: "例：〇〇だったから、〇〇について深く考えるようになった..."
  },
  {
    id: "q4",
    title: "同じ悩みを持つ人へ",
    q: "同じコンプレックスを持つ人が目の前にいたら、どんな言葉をかけますか？",
    placeholder: "例：〇〇で悩んでいる人に「〇〇」と伝えたい..."
  },
  {
    id: "q5",
    title: "武器への変換",
    q: "そのコンプレックスを「武器」と言っている人を知っていますか？\nまた、武器に変換できるとしたらどんな形がありそうですか？",
    placeholder: "例：〇〇という弱みは、〇〇という形で強みになりうると思う..."
  }
];

const HISTORY_KEY = "app07_history_v1";
...
const buildAnalysisPrompt = (userName, answers) => {
  ...
  return `あなたは「コンプレックス反転AIの専門家」です。以下のユーザーの回答を踏まえ、深く分析してください。

あなたは「コンプレックス反転の専門家」です。

### 【コンプレックスの反転フレーム】
そのコンプレックスがなぜ弱みではなく個性なのか論理的かつ共感的に示す。

### 【コンプレックスが生んだ強み】
そのコンプレックスがあったからこそ身についた能力・視点・感受性を3つ挙げる。

### 【強みとしての言語化】
コンプレックスを強みとして表現する言葉を3パターン提案する。

### 【発信・活動への転換案】
そのコンプレックスをどう仕事・発信・貢献に活かせるか具体的に提示する。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "14px 18px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: C.gold }}>コンプレックス反転AI</div>
            <div style={{ fontSize: 10, color: C.textMuted }}>コンプレックスを強みに反転させる</div>
          </div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "28px 18px 40px" }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 12, lineHeight: 1.5 }}>コンプレックス反転AI</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>コンプレックスを強みに反転させる。あなたの中に眠る答えを引き出します。</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "18px 18px 40px" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={async () => {
              const text = `【コンプレックス反転AI】${selectedHistory.date}\n\n${selectedHistory.analysis}`;
