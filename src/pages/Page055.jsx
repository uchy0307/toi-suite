...
const QUESTIONS = [
  {
    id: "q1",
    title: "関わる相手",
    q: "あなたが「第一印象シミュレーター」を意識する相手や場面を、3人/3シーン以内で挙げてください。",
    placeholder: "例: 妻、職場の後輩、義母、SNSのフォロワー..."
  },
  {
    id: "q2",
    title: "過去の出来事",
    q: "「プロフ写真や文章の印象分析」に関連する印象的な過去の出来事を1つ教えてください。",
    placeholder: "例: 5年前、〇〇さんとのやり取りで..."
  },
  {
    id: "q3",
    title: "理想の関係性",
    q: "「第一印象シミュレーター」が完璧にできるとしたら、その人間関係はどんな雰囲気になりますか？",
    placeholder: "例: お互いに気を使わずに..."
  },
  {
    id: "q4",
    title: "自分の癖",
    q: "「第一印象シミュレーター」を阻んでいる、自分の癖や思考パターンは何ですか？",
    placeholder: "例: つい強く言ってしまう、本音が言えず溜め込む..."
  },
  {
    id: "q5",
    title: "明日からの一歩",
    q: "明日からできる、「第一印象シミュレーター」に向けた小さな実験は？",
    placeholder: "例: 1日1回、相手の話を最後まで遮らずに聞く..."
  }
];

const HISTORY_KEY = "app055_history_v1";
...
const buildAnalysisPrompt = (userName, answers) => {
  ...
  return `あなたは「第一印象シミュレーターの専門家」です。以下のユーザーの回答から、プロフ写真や文章の印象分析ための深い分析を提供してください。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "14px 18px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: C.gold }}>第一印象シミュレーター</div>
            <div style={{ fontSize: 10, color: C.textMuted }}>プロフ写真や文章の印象分析</div>
          </div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "28px 18px 40px" }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 12, lineHeight: 1.5 }}>第一印象シミュレーター</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>プロフ写真や文章の印象分析。あなたの中に眠る答えを引き出します。</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "18px 18px 40px" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={async () => {
              const text = `【第一印象シミュレーター】${selectedHistory.date}\n\n${selectedHistory.analysis}`;
