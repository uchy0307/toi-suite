// === Page006.jsx === 隠れた才能診断 - δ方式 v1.0
// Auto-generated from Page004 template (Phase 1 mass deploy)
// Theme keywords: 才能, 診断, 発掘

import React, { useState, useRef, useEffect } from "react";

// ───────────────────────────────────────────────────────────
// 隠れた才能診断 v2.0 - 4階層カテゴリ＋10問ラリー＋6軸レーダー (Page006)
// δ方式実装: API呼び出しゼロ、プロンプト生成のみ
// ───────────────────────────────────────────────────────────
// テーマ: 隠れた才能診断 を構造化し、本人視点での再認識と次の一手を引き出す
// 構造:
//  - 4階層カテゴリ: 生活領域 / 感情タイプ / 思考パターン / 時間スコープ
//  - 10回会話ラリー: Q1〜Q10の段階的ヒアリング、進捗バー、前へ/次へ/スキップ
//  - 隠れた才能診断レーダー: 6軸 (認識/反復/共鳴/差別性/充実/言語化)
...
const RALLY_QUESTIONS = [
  { id: "q1", step: 1, q: "8歳のあなたは何に夢中でしたか？その時の感情も思い出してみてください", placeholder: "例：図鑑を読み漁る / 何かを集める / 1人で工作 など。「親に言われなくてもやっていたこと」" },
  { id: "q2", step: 2, q: "親に「やりなさい」と言われなくても、黙ってやっていたことは？", placeholder: "例：絵を描く / 本を読む / 何かを観察する / 友達のもめ事を仲裁する など" },
  { id: "q3", step: 3, q: "過去10年で、同じ理由で3回以上褒められたことは何ですか？具体的な場面で", placeholder: "例：「説明が分かりやすい」と3人から / 「気が利く」と何度も など" },
  { id: "q4", step: 4, q: "その褒め言葉に共通する「あなたの特徴」を一言でまとめると？", placeholder: "例：「構造化が早い」「相手の本音を引き出す」「細部に気づく」など" },
  { id: "q5", step: 5, q: "自分は自然にやっているのに、他人から「そこまでやるか？」と言われることは", placeholder: "例：プレゼン資料の作り込み / 議事録の質 / 相手の表情の読み取り など" },
  { id: "q6", step: 6, q: "その時の他人の反応は「羨望」「不思議」「呆れ」「感謝」のどれに近い？", placeholder: "羨望なら本物の才能の可能性。各反応の具体例も思い出してください" },
  { id: "q7", step: 7, q: "その才能は、誰の・どんな困りを解決していますか？", placeholder: "例：社内の混乱した会議を整理 / 部下の本音を引き出す / 顧客の言葉にできない要望を翻訳 など" },
  { id: "q8", step: 8, q: "その才能をサービスとして提供するなら、誰に・何を・どう届けたい？", placeholder: "例：中小企業の管理職に / 議事録テンプレを / 月1セッションで など。仮説で構いません" },
  { id: "q9", step: 9, q: "「私はこういう才能を持っています」を3行で宣言するなら、何と書きますか？", placeholder: "1行目：才能の名前\n2行目：その才能で何ができるか\n3行目：誰の役に立てるか" },
  { id: "q10", step: 10, q: "その宣言を、誰に・いつ・どう伝えますか？今夜できる小さな一歩は？", placeholder: "例：信頼できる同僚にLINE / SNSのプロフィールに1行追加 / 配偶者に話す など" },
];
...
const HEALTH_AXES = [
  { id: "clar", icon: "🔍", label: "認識", desc: "自分の才能をどれだけ自覚できているか" },
  { id: "ener", icon: "🔁", label: "反復", desc: "3回以上同テーマで褒められた実績の厚み" },
  { id: "conn", icon: "🤝", label: "共鳴", desc: "他者からの指摘・感謝の頻度" },
  { id: "grow", icon: "💎", label: "差別性", desc: "他人には真似しにくい度合い" },
  { id: "joy", icon: "✨", label: "充実", desc: "才能を発揮しているときの喜び" },
  { id: "peac", icon: "📣", label: "言語化", desc: "その才能を自分の言葉で語れる度合い" },
];
...
時間スコープ: ${(persona.time || []).join("・") || "(未選択)"}`;
  ...
  return `あなたは「隠れた才能診断の専門家」です。認知行動療法(CBT)・成功循環モデル・マズロー欲求段階の知見を踏まえ、以下の人物の才能を構造化し、解放への道筋を提示してください。
...
### 【本当の原因の特定】
表面的な才能の奥にある根本原因を、ペルソナ4軸と10ラリー回答を統合して論理的に示してください（300〜400字）。
...
const buildPerspectivePrompt = (userName, analysisText) => {
  return `あなたは「他者視点分析の専門家」です。以下の隠れた才能診断分析を踏まえ、3つの異なる視点からその人を見たときに何が見えるかを描写してください。
...
const buildDeepDivePrompt = (userName, analysisText, theme) => {
  return `あなたは「強み言語化の専門家」です。以下の人物の隠れた才能診断分析結果を踏まえ、特定のテーマについて深掘りした追加分析を行ってください。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "12px 16px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, boxShadow: "0 1px 6px rgba(0,0,0,0.06)", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: C.goldDim }}>隠れた才能診断</div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "24px 18px 40px", animation: "fadeUp 0.4s ease" }}>
          <div style={{ textAlign: "center", marginBottom: 24 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 8, lineHeight: 1.5 }}>隠れた才能診断 v2</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>「隠れた才能診断」を<span style={{ color: C.goldDim, fontWeight: 600 }}>4階層カテゴリ・10問ラリー・6軸レーダー</span>で構造化し、自分視点で深掘りする。CBT × 成功循環 × マズロー。</div>
          </div>
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: 18, marginBottom: 20 }}>
            {[
              ...
              { n: "02", t: "隠れた才能診断レーダー", d: "睡眠・食欲・集中・気分・対人・興味の6軸を1〜5で評価" },
      ...
      {screen === "persona" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 32px" }}>
          ...
          <button onClick={personaReady ? goToRadar : null} disabled={!personaReady}
            ...
            {personaReady ? "▶ 隠れた才能診断レーダーへ" : "まずは1項目以上選択"}
          </button>
      ...
      {screen === "result" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 32px" }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: C.goldDim, marginBottom: 12 }}>📋 隠れた才能診断分析</div>
