...
const QUESTIONS = [
  {
    id: "q1",
    title: "気になること",
    q: "「なぜそんなことが気になるの？」と言われた経験はありますか？\nあなたが自然と注目してしまうことは何ですか？",
    placeholder: "例：人の表情の微妙な変化が気になる、データのズレが目につく..."
  },
  {
    id: "q2",
    title: "引き込まれる話題",
    q: "人と話す時、自然とどんな話題に引き込まれますか？\n気づいたら熱く語っていることは何ですか？",
    placeholder: "例：〇〇の話になるとつい長くなってしまう..."
  },
  {
    id: "q3",
    title: "負けたくないもの",
    q: "趣味・仕事・日常で「これだけは負けない」と感じることは何ですか？",
    placeholder: "例：〇〇については誰にも負けたくないと思っている..."
  },
  {
    id: "q4",
    title: "何度でもやり直す",
    q: "失敗しても何度でもやり直したいと思えることは何ですか？",
    placeholder: "例：〇〇はうまくいかなくてもまたやりたくなる..."
  },
  {
    id: "q5",
    title: "あなたといると",
    q: "友人・同僚から「あなたといると〇〇になれる」と言われることはありますか？",
    placeholder: "例：〇〇さんから「あなたといると〇〇になれる」と言われた..."
  }
];

const HISTORY_KEY = "app06_history_v1";
...
const buildAnalysisPrompt = (userName, answers) => {
  ...
  return `あなたは「隠れた才能診断の専門家」です。以下のユーザーの回答を踏まえ、深く分析してください。

あなたは「才能発見の専門家」です。

### 【あなたの隠れた才能TOP3】
各才能を命名し、なぜその才能があると言えるのか根拠を示す。

### 【才能の組み合わせによるユニークポジション】
複数の才能がどう組み合わさって他にない強みを生むか説明する。

### 【才能を活かせる場面・仕事5選】
具体的にどんな場面でその才能を発揮できるか5つ提示する。

### 【今週のアクション】
1つの才能を意識的に使う小さなチャレンジを提案する。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "14px 18px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: C.gold }}>隠れた才能診断</div>
            <div style={{ fontSize: 10, color: C.textMuted }}>見逃している自分の才能を可視化する</div>
          </div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "28px 18px 40px" }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 12, lineHeight: 1.5 }}>隠れた才能診断</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>見逃している自分の才能を可視化する。あなたの中に眠る答えを引き出します。</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "18px 18px 40px" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={async () => {
              const text = `【隠れた才能診断】${selectedHistory.date}\n\n${selectedHistory.analysis}`;
