...
const QUESTIONS = [
  {
    id: "q1",
    title: "今の選択の動機",
    q: "今の仕事・生活・人間関係について、「自分で選んだ」と「誰かの期待に応えた」の割合はどのくらいですか？",
    placeholder: "例：自分で選んだ〇%、誰かの期待〇%..."
  },
  {
    id: "q2",
    title: "本当はやりたくないこと",
    q: "「本当はやりたくないけどやっていること」は何ですか？
なぜやり続けているのですか？",
    placeholder: "例：〇〇は本当はやりたくない。でも〇〇だからやっている..."
  },
  {
    id: "q3",
    title: "SNSと本当の自分",
    q: "SNSや周りに見せている自分と、本当の自分の差はどのくらいありますか？",
    placeholder: "例：外では〇〇に見せているが、実際は〇〇..."
  },
  {
    id: "q4",
    title: "べき思考の出所",
    q: "「〇〇であるべき」という思い込みがあれば書いてください。
その思い込みは誰から来ていますか？",
    placeholder: "例：〇〇であるべきという思い込みがある。これは〇〇から..."
  },
  {
    id: "q5",
    title: "誰の目もなければ",
    q: "誰の目も気にしなくていいとしたら、今の生活で何を変えますか？",
    placeholder: "例：誰も見ていないなら〇〇をやめて、〇〇を始める..."
  }
];

const HISTORY_KEY = "app14_history_v1";
...
const buildAnalysisPrompt = (userName, answers, mode = "deep") => {
  ...
  return `あなたは「偽りの自分チェッカーの専門家」です。以下のユーザーの回答を踏まえ、深く分析してください。

あなたは「自分軸・他人軸の専門家」です。

### 【自分軸スコア（10段階）】
回答から自分軸の強度を10段階で判定し根拠を示す。

### 【他人軸が強い領域の特定】
どの領域で他人軸になっているか明確にする。

### 【他人軸になった根本原因】
その思考パターンがいつ・なぜ形成されたか分析する。

### 【自分軸を取り戻すステップ】
段階的に自分軸に移行するための具体的なステップを3つ示す。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "14px 18px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: C.gold }}>偽りの自分チェッカー</div>
            <div style={{ fontSize: 10, color: C.textMuted }}>偽りの自分から本当の自分を見分ける</div>
          </div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "28px 18px 40px" }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 12, lineHeight: 1.5 }}>偽りの自分チェッカー</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>偽りの自分から本当の自分を見分ける。あなたの中に眠る答えを引き出します。</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "18px 18px 40px" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={async () => {
              const text = `【偽りの自分チェッカー】${selectedHistory.date}\n\n${selectedHistory.analysis}`;
