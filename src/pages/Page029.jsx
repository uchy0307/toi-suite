...
const QUESTIONS = [
  {
    id: "q1",
    title: "具体的なシーン",
    q: "「プレゼン緊張解消」が必要だと最近感じた具体的なシーンを教えてください。誰と、どんな場面でしたか？",
    placeholder: "例: 先週の会議で〇〇の決断を迫られて..."
  },
  {
    id: "q2",
    title: "過去の経験",
    q: "これまでのキャリアで、「本番で実力を出す思考整理」に関連する成功体験/失敗体験を1つずつ教えてください。",
    placeholder: "例: 〇〇プロジェクトで〇〇したら..."
  },
  {
    id: "q3",
    title: "理想の状態",
    q: "「プレゼン緊張解消」が完璧に機能している1年後の自分を想像してください。何が変わっていますか？",
    placeholder: "例: 1年後、〇〇できる自分になっていて..."
  },
  {
    id: "q4",
    title: "障害・ボトルネック",
    q: "「プレゼン緊張解消」を妨げている内的・外的要因は何ですか？正直に書いてください。",
    placeholder: "例: 内的: 自信不足、外的: 上司の理解..."
  },
  {
    id: "q5",
    title: "最初の一歩",
    q: "今週、「プレゼン緊張解消」のために試せる最小の行動は何ですか？",
    placeholder: "例: 〇〇の本を1章読む、〇〇さんに話を聞く..."
  }
];

const HISTORY_KEY = "app029_history_v1";
...
const buildAnalysisPrompt = (userName, answers) => {
  ...
  return `あなたは「プレゼン緊張解消の専門家」です。以下のユーザーの回答から、本番で実力を出す思考整理ための深い分析を提供してください。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "14px 18px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: C.gold }}>プレゼン緊張解消</div>
            <div style={{ fontSize: 10, color: C.textMuted }}>本番で実力を出す思考整理</div>
          </div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "28px 18px 40px" }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 12, lineHeight: 1.5 }}>プレゼン緊張解消</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>本番で実力を出す思考整理。あなたの中に眠る答えを引き出します。</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "18px 18px 40px" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={async () => {
              const text = `【プレゼン緊張解消】${selectedHistory.date}\n\n${selectedHistory.analysis}`;
