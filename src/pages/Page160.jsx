...
const QUESTIONS = [
  {
    id: "q1",
    title: "ニーズの場面",
    q: "「メンタル回復セルフ質問」が必要となる、最近の具体的シーンを教えてください。",
    placeholder: "例: 先週、〇〇の場面で〇〇しようとして..."
  },
  {
    id: "q2",
    title: "過去の経験",
    q: "これまで「どん底の時に光を見る」に関連する印象的な経験は？",
    placeholder: "例: 〇〇の場面で〇〇した結果..."
  },
  {
    id: "q3",
    title: "理想の機能",
    q: "「メンタル回復セルフ質問」が完璧に機能した状態を描写してください。",
    placeholder: "例: 〇〇が解決し、〇〇という結果が..."
  },
  {
    id: "q4",
    title: "障害",
    q: "「メンタル回復セルフ質問」を妨げている要因は何ですか？",
    placeholder: "例: 知識不足、時間不足、相手側の理解..."
  },
  {
    id: "q5",
    title: "最初の一歩",
    q: "今すぐ・今週試せる、「メンタル回復セルフ質問」の小さな一歩は？",
    placeholder: "例: 〇〇について調べる、〇〇に相談する..."
  }
];

const HISTORY_KEY = "app160_history_v1";
...
const buildAnalysisPrompt = (userName, answers) => {
  ...
  return `あなたは「メンタル回復セルフ質問の専門家」です。以下のユーザーの回答から、どん底の時に光を見るための深い分析を提供してください。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "14px 18px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: C.gold }}>メンタル回復セルフ質問</div>
            <div style={{ fontSize: 10, color: C.textMuted }}>どん底の時に光を見る</div>
          </div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "28px 18px 40px" }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 12, lineHeight: 1.5 }}>メンタル回復セルフ質問</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>どん底の時に光を見る。あなたの中に眠る答えを引き出します。</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "18px 18px 40px" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={async () => {
              const text = `【メンタル回復セルフ質問】${selectedHistory.date}\n\n${selectedHistory.analysis}`;
