...
const QUESTIONS = [
  {
    id: "q1",
    title: "最近の重要な決断",
    q: "最近した「重要な決断」を3つ挙げて、それぞれ何を基準に決めたか教えてください。",
    placeholder: "例：1.〇〇を決めた。基準は〇〇 2.〇〇 3.〇〇..."
  },
  {
    id: "q2",
    title: "他者の目が気になる場面",
    q: "「人にどう思われるか」が気になって行動を変えた経験はありますか？",
    placeholder: "例：〇〇の時、〇〇と思われたくて〇〇を選んだ..."
  },
  {
    id: "q3",
    title: "反対されてもやること",
    q: "誰かに反対されても「これはやる」と決めたことがありますか？\nその時、何が決断の根拠でしたか？",
    placeholder: "例：〇〇に反対されたが、〇〇だったのでやると決めた..."
  },
  {
    id: "q4",
    title: "多数派との違い",
    q: "自分の意見と多数派が違う時、どう行動する傾向がありますか？",
    placeholder: "例：〇〇の時、多数派と違う意見を持ったが〇〇した..."
  },
  {
    id: "q5",
    title: "本音の意思決定",
    q: "「自分がどうしたいか」を問う前に「どうすべきか」を考えていませんか？\n最近の例を1つ教えてください。",
    placeholder: "例：〇〇の時、まず「すべき」を考えてから「したい」を考えた..."
  }
];

const HISTORY_KEY = "app19_history_v1";
...
const buildAnalysisPrompt = (userName, answers) => {
  ...
  return `あなたは「自分軸vs他人軸判定の専門家」です。以下のユーザーの回答を踏まえ、深く分析してください。

あなたは「自分軸の専門家」です。

### 【自分軸スコア（10段階）】
回答全体から自分軸の強度を10段階で判定し根拠を示す。

### 【意思決定パターンの分析】
どんな場面で自分軸・他人軸になっているかパターンを整理する。

### 【他人軸の根本原因】
なぜその思考パターンが形成されたか心理的背景を分析する。

### 【自分軸強化の3ステップ】
自分軸を少しずつ強化するための段階的なアプローチを示す。

### 【今週の自分軸実験】
小さな決断で「自分がどうしたいか」を優先する練習を提案する。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "14px 18px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: C.gold }}>自分軸vs他人軸判定</div>
            <div style={{ fontSize: 10, color: C.textMuted }}>他人軸で生きてる自分に気づき、自分軸を取り戻す</div>
          </div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "28px 18px 40px" }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 12, lineHeight: 1.5 }}>自分軸vs他人軸判定</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>他人軸で生きてる自分に気づき、自分軸を取り戻す。あなたの中に眠る答えを引き出します。</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "18px 18px 40px" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={async () => {
              const text = `【自分軸vs他人軸判定】${selectedHistory.date}\n\n${selectedHistory.analysis}`;
