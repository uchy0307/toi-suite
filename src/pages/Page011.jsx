...
const QUESTIONS = [
  {
    id: "q1",
    title: "今週のできたこと",
    q: "今週、小さなことでも「できたこと」を10個書き出してください。
「当たり前」のことでも構いません。",
    placeholder: "例：1.朝決めた時間に起きた 2.〇〇に返信した 3...."
  },
  {
    id: "q2",
    title: "乗り越えてきたこと",
    q: "過去3年で「これは大変だった」「よく乗り越えられた」と思うことは何ですか？",
    placeholder: "例：〇〇という状況を乗り越えた..."
  },
  {
    id: "q3",
    title: "比較の基準",
    q: "「自分はダメだ」と思う時、誰と・何と比べていますか？
その基準は本当に公平だと思いますか？",
    placeholder: "例：〇〇と比べて自分はダメだと感じる..."
  },
  {
    id: "q4",
    title: "他者への言葉",
    q: "親友が同じ状況・同じ悩みを抱えていたら、あなたはどんな言葉をかけますか？",
    placeholder: "例：友人には「〇〇」と伝えると思う..."
  },
  {
    id: "q5",
    title: "自分の良いところ",
    q: "自分の良いところを3つ挙げてください。
難しければ、他者から言われたことを思い出してみてください。",
    placeholder: "例：1.〇〇 2.〇〇 3.〇〇"
  }
];

const HISTORY_KEY = "app11_history_v1";
...
const buildAnalysisPrompt = (userName, answers, mode = "deep") => {
  ...
  return `あなたは「自己肯定感ブースターの専門家」です。以下のユーザーの回答を踏まえ、深く分析してください。

あなたは「自己肯定感の専門家」です。

### 【あなたの「できたこと」の再定義】
挙げられたできたことの意味・価値を深く言語化する。

### 【あなたが持つ実力の証拠】
回答から見えるこの人の強さ・能力を客観的に示す。

### 【自己評価の歪みの修正】
比較の基準の問題点を優しく指摘し公平な自己評価の視点を提供する。

### 【自己肯定感を育てる習慣】
毎日続けられる小さな習慣を3つ提案する。

### 【今日の自分への手紙】
この人が今日の自分に送るべき温かいメッセージを1段落書く。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "14px 18px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: C.gold }}>自己肯定感ブースター</div>
            <div style={{ fontSize: 10, color: C.textMuted }}>「できたこと」を再認識し自己肯定感を育てる</div>
          </div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "28px 18px 40px" }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 12, lineHeight: 1.5 }}>自己肯定感ブースター</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>「できたこと」を再認識し自己肯定感を育てる。あなたの中に眠る答えを引き出します。</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "18px 18px 40px" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={async () => {
              const text = `【自己肯定感ブースター】${selectedHistory.date}\n\n${selectedHistory.analysis}`;
