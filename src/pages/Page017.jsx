...
const QUESTIONS = [
  {
    id: "q1",
    title: "積み上げてきた経験",
    q: "今の立場・役割を得るまでに積み上げてきた経験・努力・乗り越えた困難を、具体的に3つ以上教えてください。",
    placeholder: "例：〇年間、〇〇を続けてきた。〇〇という困難を乗り越えた..."
  },
  {
    id: "q2",
    title: "乗り越えた壁",
    q: "「絶対に無理だ」「自分には難しすぎる」と思ったのに、実際には乗り越えられた経験を教えてください。",
    placeholder: "例：〇〇という状況で、私は〇〇したことで..."
  },
  {
    id: "q3",
    title: "誰かへの貢献",
    q: "誰かから「助かった」「ありがとう」「あなたがいてよかった」と言われた具体的な場面を、2〜3個教えてください。",
    placeholder: "例：〇〇の時、〇〇さんから〇〇と言われた..."
  },
  {
    id: "q4",
    title: "自分だけの強み",
    q: "周囲の人が苦労しているのに、あなたは比較的楽にできること・自然にやっていることは何ですか？",
    placeholder: "例：私は〇〇が比較的得意で、周りから〇〇について頼まれることが多い..."
  }
];

const HISTORY_KEY = "app17_history_v1";
...
const buildAnalysisPrompt = (userName, answers) => {
  ...
  return `あなたは「インポスター症候群対策の専門家」です。以下のユーザーの回答を踏まえ、深く分析してください。

あなたは「インポスター症候群専門のサポートカウンセラー」です。認知行動療法（CBT）と自己効力感理論を基盤に、ユーザーが「自分なんて」という不安を論理的に解消できるようサポートします。

## 分析の構造（必ず全セクションを書き切ること）

### 【あなたのインポスター症候群の特徴】
症状スコアと具体的な症状から、その人特有のパターンを分析する（2〜3段落）。

### 【認知の歪みの特定】
その人の回答から見られる認知の歪みを具体的に示す（全か無か思考・感情的推論等）。

### 【あなたが持つ実力の証拠】
証拠収集の回答を基に、実力を客観的な視点から論述する。「たまたま」では説明できない要素を明示する。

### 【思考の書き換えフレーム】
「インポスター思考」→「事実に基づく思考」への具体的な書き換え例を3つ示す。

### 【今日から始める実践】
毎日続けられる小さな習慣を2〜3個、具体的に提案する。

日本語で、温かみのある知的な文体で書く。1000〜1400字程度。途中で終わらないこと。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "14px 18px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: C.gold }}>インポスター症候群対策</div>
            <div style={{ fontSize: 10, color: C.textMuted }}>「自分なんて」を論理的に解消する</div>
          </div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "28px 18px 40px" }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 12, lineHeight: 1.5 }}>インポスター症候群対策</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>「自分なんて」を論理的に解消する。あなたの中に眠る答えを引き出します。</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "18px 18px 40px" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={async () => {
              const text = `【インポスター症候群対策】${selectedHistory.date}\n\n${selectedHistory.analysis}`;
