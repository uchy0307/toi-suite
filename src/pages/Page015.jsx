...
const QUESTIONS = [
  {
    id: "q1",
    title: "集中できる時間帯",
    q: "最も集中できる時間帯はいつですか？\n朝・昼・夜・深夜——具体的に何時〜何時頃ですか？",
    placeholder: "例：〇時〜〇時が一番集中できる..."
  },
  {
    id: "q2",
    title: "集中できる環境",
    q: "集中できる場所・環境の特徴は何ですか？\n音・光・温度・一人/複数——具体的に教えてください。",
    placeholder: "例：静かな場所より〇〇な場所の方が集中できる..."
  },
  {
    id: "q3",
    title: "ゾーン前のルーティン",
    q: "ゾーンに入る直前に何かしていますか？\n無意識にやっていることがあれば教えてください。",
    placeholder: "例：〇〇をしてから作業に入ると集中しやすい..."
  },
  {
    id: "q4",
    title: "集中を切るもの",
    q: "集中が切れる主なトリガーは何ですか？\n内的と外的に分けて教えてください。",
    placeholder: "例：内的：〇〇が気になる。外的：〇〇があると切れる..."
  },
  {
    id: "q5",
    title: "過去最高の集中状態",
    q: "過去で最も集中できた経験はいつ・何をしていた時ですか？",
    placeholder: "例：〇〇の時が過去最高だった..."
  }
];

const HISTORY_KEY = "app15_history_v1";
...
const buildAnalysisPrompt = (userName, answers) => {
  ...
  return `あなたは「集中力プロファイラーの専門家」です。以下のユーザーの回答を踏まえ、深く分析してください。

あなたは「集中力の専門家」です。

### 【あなたの集中力プロファイル】
最適な集中状態の全体像を時間・環境・状態でまとめる。

### 【ゾーンに入る5条件】
この人がゾーンに入るための具体的な条件を5つ挙げる。

### 【集中を妨げる要因の除去リスト】
集中を妨げているものを特定し各々の対処法を示す。

### 【理想の集中環境設計】
仕事・作業環境を最適化するための具体的な提案を行う。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "14px 18px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: C.gold }}>集中力プロファイラー</div>
            <div style={{ fontSize: 10, color: C.textMuted }}>あなたの集中タイプを知り、最高のパフォーマンスへ</div>
          </div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "28px 18px 40px" }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 12, lineHeight: 1.5 }}>集中力プロファイラー</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>あなたの集中タイプを知り、最高のパフォーマンスへ。あなたの中に眠る答えを引き出します。</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "18px 18px 40px" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={async () => {
              const text = `【集中力プロファイラー】${selectedHistory.date}\n\n${selectedHistory.analysis}`;
