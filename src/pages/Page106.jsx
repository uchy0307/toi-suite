...
const QUESTIONS = [
  {
    id: "q1",
    title: "数字で現状",
    q: "「老後不安の解体」について、今のあなたの状況を数字で書いてください。",
    placeholder: "例: 月収〇万円、貯金〇万円、月の自由費〇万円..."
  },
  {
    id: "q2",
    title: "過去のお金経験",
    q: "これまで「必要な金額と仲間を具体化」に関連して、成功・失敗した経験は？",
    placeholder: "例: 〇万円投資して〇万円になった、衝動買いで..."
  },
  {
    id: "q3",
    title: "理想の家計",
    q: "「老後不安の解体」が解決した時、家計や人生はどう変わっていますか？",
    placeholder: "例: 月〇万円自由になり、〇〇に使える..."
  },
  {
    id: "q4",
    title: "心理ブロック",
    q: "「老後不安の解体」を妨げている、自分の中の心理ブロックは？",
    placeholder: "例: お金を受け取る罪悪感、貯金できない焦り..."
  },
  {
    id: "q5",
    title: "今月の最小行動",
    q: "今月できる、「老後不安の解体」に向けた最小の行動は？",
    placeholder: "例: 家計簿を3日記録する、銀行口座を1つ整理する..."
  }
];

const HISTORY_KEY = "app106_history_v1";
...
const buildAnalysisPrompt = (userName, answers) => {
  ...
  return `あなたは「老後不安の解体の専門家」です。以下のユーザーの回答から、必要な金額と仲間を具体化ための深い分析を提供してください。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "14px 18px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: C.gold }}>老後不安の解体</div>
            <div style={{ fontSize: 10, color: C.textMuted }}>必要な金額と仲間を具体化</div>
          </div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "28px 18px 40px" }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 12, lineHeight: 1.5 }}>老後不安の解体</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>必要な金額と仲間を具体化。あなたの中に眠る答えを引き出します。</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "18px 18px 40px" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={async () => {
              const text = `【老後不安の解体】${selectedHistory.date}\n\n${selectedHistory.analysis}`;
