// === Page106.jsx === 老後不安の解体 - δ方式 v1.0
// Auto-generated from Page004 template (Phase 1 mass deploy)
// Theme keywords: 老後, 不安, 解体

import React, { useState, useRef, useEffect } from "react";

// ───────────────────────────────────────────────────────────
// 老後不安の解体 v2.0 - 4階層カテゴリ＋10問ラリー＋6軸レーダー (Page106)
// δ方式実装: API呼び出しゼロ、プロンプト生成のみ
// ───────────────────────────────────────────────────────────
// テーマ: 老後不安の解体 を構造化し、本人視点での再認識と次の一手を引き出す
// 構造:
//  - 4階層カテゴリ: 生活領域 / 感情タイプ / 思考パターン / 時間スコープ
//  - 10回会話ラリー: Q1〜Q10の段階的ヒアリング、進捗バー、前へ/次へ/スキップ
//  - 老後不安の解体レーダー: 6軸 (睡眠/食欲/集中/気分/対人/興味)
...
const RALLY_QUESTIONS = [
  { id: "q1", step: 1, q: "今、「老後」について頭の中にあることを全部書き出してください。", placeholder: "例：老後が気がかり、最近老後について思い出すこと、引っかかっている言葉... 小さなことでも全部" },
  ...
  { id: "q4", step: 4, q: "これまでに老後についてどう取り組んできましたか？効いた・効かなかったのは？", placeholder: "例：本を読んだ / 誰かに相談した / 試したが続かなかった など" },
  { id: "q5", step: 5, q: "もし「老後」が理想の状態に近づいたら、何が一番変わると思いますか？", placeholder: "例：朝の気分が変わる / 周りとの関係が変わる / 自信がつく など" },
  { id: "q6", step: 6, q: "「老後」のうち、自分でコントロールできる部分はどこですか？できない部分は？", placeholder: "コントロールできる：...\nコントロールできない：..." },
  ...
  { id: "q9", step: 9, q: "1年後、「老後」はどうなっていそうですか？残っているとしたら何が変わっていますか？", placeholder: "例：解決している / 形を変えて残っている / 別の課題に置き換わる など" },
...
時間スコープ: ${(persona.time || []).join("・") || "(未選択)"}`;
  ...
  return `あなたは「「老後不安の解体」の専門家」です。認知行動療法(CBT)・成功循環モデル・マズロー欲求段階の知見を踏まえ、以下の人物の老後を構造化し、解放への道筋を提示してください。
...
### 【本当の原因の特定】
表面的な老後の奥にある根本原因を、ペルソナ4軸と10ラリー回答を統合して論理的に示してください（300〜400字）。
...
const buildPerspectivePrompt = (userName, analysisText) => {
  return `あなたは「他者視点分析の専門家」です。以下の老後不安の解体分析を踏まえ、3つの異なる視点からその人を見たときに何が見えるかを描写してください。
...
const buildDeepDivePrompt = (userName, analysisText, theme) => {
  return `あなたは「強み言語化の専門家」です。以下の人物の老後不安の解体分析結果を踏まえ、特定のテーマについて深掘りした追加分析を行ってください。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "12px 16px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, boxShadow: "0 1px 6px rgba(0,0,0,0.06)", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: C.goldDim }}>老後不安の解体</div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "24px 18px 40px", animation: "fadeUp 0.4s ease" }}>
          <div style={{ textAlign: "center", marginBottom: 24 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 8, lineHeight: 1.5 }}>老後不安の解体 v2</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>「老後不安の解体」を<span style={{ color: C.goldDim, fontWeight: 600 }}>4階層カテゴリ・10問ラリー・6軸レーダー</span>で構造化し、自分視点で深掘りする。CBT × 成功循環 × マズロー。</div>
          </div>
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: 18, marginBottom: 20 }}>
            {[
              ...
              { n: "02", t: "老後不安の解体レーダー", d: "睡眠・食欲・集中・気分・対人・興味の6軸を1〜5で評価" },
      ...
      {screen === "persona" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 32px" }}>
          ...
          <button onClick={personaReady ? goToRadar : null} disabled={!personaReady}
            ...
            {personaReady ? "▶ 老後不安の解体レーダーへ" : "まずは1項目以上選択"}
          </button>
      ...
      {screen === "result" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 32px" }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: C.goldDim, marginBottom: 12 }}>📋 老後不安の解体分析</div>
