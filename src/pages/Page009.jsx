...
const QUESTIONS = [
  {
    id: "q1",
    title: "今日のハイライト",
    q: "今日（または最近）、最も気分が良かった瞬間はいつ・何をしていた時ですか？",
    placeholder: "例：〇時頃、〇〇をしていた時に気分が上がった..."
  },
  {
    id: "q2",
    title: "今日のロウポイント",
    q: "今日（または最近）、最も気分が下がった瞬間はいつ・何をしていた時ですか？\nその時どんな感情でしたか？",
    placeholder: "例：〇時頃、〇〇の時に気分が下がった。感情は〇〇..."
  },
  {
    id: "q3",
    title: "感情の分類",
    q: "最もネガティブだった感情は「怒り・不安・悲しみ・孤独・焦り・恥・疲労」のどれに近いですか？\nそれが生じた原因は何だと思いますか？",
    placeholder: "例：〇〇に近い。原因は〇〇だと思う..."
  },
  {
    id: "q4",
    title: "トリガーの特定",
    q: "その感情を引き起こしたのは「人・場所・出来事・思考・体調」のどれですか？",
    placeholder: "例：〇〇（人/場所/出来事）が原因だった..."
  },
  {
    id: "q5",
    title: "パターンの確認",
    q: "この感情パターン、過去にも繰り返していると思いますか？\nいつ頃から、どんな場面で繰り返していますか？",
    placeholder: "例：〇〇の時に似たようなことがある..."
  }
];

const HISTORY_KEY = "app09_history_v1";
...
const buildAnalysisPrompt = (userName, answers) => {
  ...
  return `あなたは「感情ログ分析の専門家」です。以下のユーザーの回答を踏まえ、深く分析してください。

あなたは「感情ログ分析の専門家」です。

### 【感情ログの構造化】
今日の感情の流れを時系列・強度・原因で整理する。

### 【ストレス要因TOP3】
繰り返されているストレスの根本要因を特定する。

### 【感情パターンの分析】
この人特有の感情反応パターンを明らかにする。

### 【感情の取扱説明書】
この人が感情をうまく扱うための具体的な3つのコツを提案する。

### 【明日の一手】
明日から実践できる感情管理の小さな習慣を提示する。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "14px 18px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: C.gold }}>感情ログ分析</div>
            <div style={{ fontSize: 10, color: C.textMuted }}>感情の波を観察し、自分のパターンを掴む</div>
          </div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "28px 18px 40px" }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 12, lineHeight: 1.5 }}>感情ログ分析</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>感情の波を観察し、自分のパターンを掴む。あなたの中に眠る答えを引き出します。</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "18px 18px 40px" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={async () => {
              const text = `【感情ログ分析】${selectedHistory.date}\n\n${selectedHistory.analysis}`;
