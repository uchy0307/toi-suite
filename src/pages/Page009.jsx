...
const RALLY_QUESTIONS = [
  { id: "q1", step: 1, q: "直近1週間で最も心を消耗した瞬間は、いつ・どこ・誰との何でしたか？", placeholder: "例：水曜夜の会議で〇〇と言われた / 金曜朝の家族とのやり取り など、具体的に" },
  { id: "q2", step: 2, q: "その瞬間の「表面の感情」を一言で（怒り/悲しみ/虚しさ/焦り 等）", placeholder: "1つに絞らなくてOK。複数が重なっていたなら全て書き出して" },
  { id: "q3", step: 3, q: "その表面感情の裏に隠れていた「本当の願い」は何でしたか？", placeholder: "例：尊重されたかった / 役に立ちたかった / 安心したかった / 認められたかった など" },
  { id: "q4", step: 4, q: "月曜朝・水曜夕方・金曜夜、それぞれの自分のモードは違いますか？", placeholder: "曜日や時間帯で自分のモードがどう変わるか、思い当たることを" },
  { id: "q5", step: 5, q: "周りの人が「あなたのモードが読めない」と感じる時間帯はどこですか？", placeholder: "家族・部下・同僚から「今日のあなた、よく分からない」と言われそうな時間帯" },
  { id: "q6", step: 6, q: "その時間帯、自分の中で何が起きていますか？（疲労？緊張？空白？）", placeholder: "言語化しにくくてもOK。身体感覚で書いてもいい" },
  { id: "q7", step: 7, q: "直近1ヶ月で、感情の波が「予想通り」だった瞬間と、意外だった瞬間は？", placeholder: "予想通り：____\n意外：____\nそれぞれ1つずつ" },
  { id: "q8", step: 8, q: "感情ログを取り始める意義を、自分の言葉で説明するなら？", placeholder: "なぜ自分は感情を記録するのか。1〜2行で" },
  { id: "q9", step: 9, q: "50代までに育てたい「感情との付き合い方」は、どんな質感のもの？", placeholder: "例：穏やか / 凛とした / 流れに任せる / 嵐の中でも軸がある など" },
  { id: "q10", step: 10, q: "今夜、感情ログを書き始める最小単位の一行は？", placeholder: "例：「今日のモード：____」だけでもOK。3秒で書ける形を決めて" },
];
...
const HEALTH_AXES = [
  { id: "clar", icon: "👁️", label: "認識", desc: "自分の感情に気づける度合い" },
  { id: "ener", icon: "✏️", label: "言語化", desc: "感情を言葉にできる度合い" },
  { id: "conn", icon: "🫂", label: "受容", desc: "どんな感情も否定せず迎え入れられる度合い" },
  { id: "grow", icon: "🔁", label: "パターン", desc: "繰り返しの波に気づける度合い" },
  { id: "joy", icon: "🍵", label: "質感", desc: "穏やかさを感じられる頻度" },
  { id: "peac", icon: "🔮", label: "予測", desc: "波を予測して備えられる度合い" },
];
...
時間スコープ: ${(persona.time || []).join("・") || "(未選択)"}`;
  ...
  return `あなたは「感情ログ分析の専門家」です。認知行動療法(CBT)・成功循環モデル・マズロー欲求段階の知見を踏まえ、以下の人物の感情を構造化し、解放への道筋を提示してください。
...
### 【本当の原因の特定】
表面的な感情の奥にある根本原因を、ペルソナ4軸と10ラリー回答を統合して論理的に示してください（300〜400字）。
...
const buildPerspectivePrompt = (userName, analysisText) => {
  return `あなたは「他者視点分析の専門家」です。以下の感情ログ分析分析を踏まえ、3つの異なる視点からその人を見たときに何が見えるかを描写してください。
...
const buildDeepDivePrompt = (userName, analysisText, theme) => {
  return `あなたは「強み言語化の専門家」です。以下の人物の感情ログ分析分析結果を踏まえ、特定のテーマについて深掘りした追加分析を行ってください。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "12px 16px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, boxShadow: "0 1px 6px rgba(0,0,0,0.06)", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: C.goldDim }}>感情ログ分析</div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "24px 18px 40px", animation: "fadeUp 0.4s ease" }}>
          <div style={{ textAlign: "center", marginBottom: 24 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 8, lineHeight: 1.5 }}>感情ログ分析 v2</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>「感情ログ分析」を<span style={{ color: C.goldDim, fontWeight: 600 }}>4階層カテゴリ・10問ラリー・6軸レーダー</span>で構造化し、自分視点で深掘りする。CBT × 成功循環 × マズロー。</div>
          </div>
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: 18, marginBottom: 20 }}>
            {[
              ...
              { n: "02", t: "感情ログ分析レーダー", d: "睡眠・食欲・集中・気分・対人・興味の6軸を1〜5で評価" },
      ...
      {screen === "persona" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 32px" }}>
          ...
          <button onClick={personaReady ? goToRadar : null} disabled={!personaReady}
            ...
            {personaReady ? "▶ 感情ログ分析レーダーへ" : "まずは1項目以上選択"}
          </button>
      ...
      {screen === "result" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 32px" }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: C.goldDim, marginBottom: 12 }}>📋 感情ログ分析分析</div>
