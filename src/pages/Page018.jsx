...
const QUESTIONS = [
  {
    id: "q1",
    title: "挑戦したいこと",
    q: "今、挑戦したいけど「怖くてできていないこと」は何ですか？",
    placeholder: "例：〇〇を始めたいが、怖くてできていない..."
  },
  {
    id: "q2",
    title: "恐怖の正体",
    q: "その恐怖を具体的に言語化してください。\n失敗・恥・損失・拒絶——何が怖いのですか？",
    placeholder: "例：怖いのは〇〇。具体的には〇〇ということが起きるのが怖い..."
  },
  {
    id: "q3",
    title: "最悪と最高のシナリオ",
    q: "挑戦した場合の「最悪のシナリオ」と「最高のシナリオ」を両方書いてください。\nそれぞれの確率も教えてください。",
    placeholder: "最悪：〇〇（確率〇%）\n最高：〇〇（確率〇%）"
  },
  {
    id: "q4",
    title: "挑戦しなかった未来",
    q: "もし挑戦しないまま1年・3年・10年が過ぎたら、どうなっていると思いますか？",
    placeholder: "例：1年後は〇〇、3年後は〇〇、10年後は〇〇になっていそう..."
  },
  {
    id: "q5",
    title: "最小の一歩",
    q: "その挑戦に向けて「5分でできる最小の一歩」は何ですか？",
    placeholder: "例：まず〇〇に電話する、〇〇を検索する..."
  }
];

const HISTORY_KEY = "app18_history_v1";
...
const buildAnalysisPrompt = (userName, answers) => {
  ...
  return `あなたは「コンフォートゾーン脱出の専門家」です。以下のユーザーの回答を踏まえ、深く分析してください。

あなたは「変化・挑戦の専門家」です。

### 【恐怖の構造化】
何が恐怖の本質なのかを分析しその恐怖が合理的かどうか検証する。

### 【リスク・リターン分析】
最悪・最高シナリオを客観的に分析し実際のリスクがどの程度かを示す。

### 【挑戦しない場合のコスト】
現状維持を続けた場合の機会コスト・感情コストを明確にする。

### 【段階的挑戦ロードマップ】
挑戦を5段階に分けたロードマップを作成する。

### 【今週の最初の一歩】
最も小さな一歩を具体的に示し実行を後押しする。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "14px 18px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: C.gold }}>コンフォートゾーン脱出</div>
            <div style={{ fontSize: 10, color: C.textMuted }}>心地よいゾーンから一歩踏み出す勇気を作る</div>
          </div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "28px 18px 40px" }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 12, lineHeight: 1.5 }}>コンフォートゾーン脱出</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>心地よいゾーンから一歩踏み出す勇気を作る。あなたの中に眠る答えを引き出します。</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "18px 18px 40px" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={async () => {
              const text = `【コンフォートゾーン脱出】${selectedHistory.date}\n\n${selectedHistory.analysis}`;
