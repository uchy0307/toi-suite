...
// ───────────────────────────────────────────────────────────
// YouTube企画・フック作成 (App.jsx) - δ方式 v1.0
...
const QUESTIONS = [
  {
    id: "q1",
    title: "最新プロジェクト",
    q: "「YouTube企画・フック作成」を使いたい、最新の作品・プロジェクトは何ですか？",
    placeholder: "例: 来月の個展、新曲制作、note記事..."
  },
  {
    id: "q2",
    title: "過去の手応え",
    q: "「最初の5秒で手を止めさせる」を試した過去の作品で、手応えがあったのは？",
    placeholder: "例: 〇〇という曲を作った時、〇〇という写真を撮った時..."
  },
  {
    id: "q3",
    title: "完璧な表現",
    q: "「YouTube企画・フック作成」が完璧にできた時、作品はどう変わりますか？",
    placeholder: "例: 観た人が泣く、自然と買いたくなる..."
  },
  {
    id: "q4",
    title: "内なる枠",
    q: "「YouTube企画・フック作成」を阻む、自分の中の「自己批判」や「枠」は？",
    placeholder: "例: 完璧主義、人と比べる癖、商業的に成立しないと..."
  },
  {
    id: "q5",
    title: "実験プロジェクト",
    q: "今週、「YouTube企画・フック作成」を試す小さな実験プロジェクトは？",
    placeholder: "例: 30分でラフを5枚描く、1つの音だけで曲を作る..."
  }
];

const HISTORY_KEY = "app116_history_v1";
...
const buildAnalysisPrompt = (userName, answers) => {
  ...
  return `あなたは「YouTube企画・フック作成の専門家」です。以下のユーザーの回答から、最初の5秒で手を止めさせるための深い分析を提供してください。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "14px 18px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: C.gold }}>YouTube企画・フック作成</div>
            <div style={{ fontSize: 10, color: C.textMuted }}>最初の5秒で手を止めさせる</div>
          </div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "28px 18px 40px" }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 12, lineHeight: 1.5 }}>YouTube企画・フック作成</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>最初の5秒で手を止めさせる。あなたの中に眠る答えを引き出します。</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "18px 18px 40px" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={async () => {
              const text = `【YouTube企画・フック作成】${selectedHistory.date}\n\n${selectedHistory.analysis}`;
