...
const QUESTIONS = [
  {
    id: "q1",
    title: "いまの状態",
    q: "「将来不安シミュレーション」について、今のあなたの状態を5段階で評価して、その理由を書いてください。",
    placeholder: "例: 3点。〇〇は満たされているが〇〇が..."
  },
  {
    id: "q2",
    title: "過去の効果",
    q: "これまで「不安の正体を具体化し対策」に関連して、効果があった習慣・経験は？",
    placeholder: "例: 〇〇を始めてから眠れるようになった..."
  },
  {
    id: "q3",
    title: "満たされた自分",
    q: "「将来不安シミュレーション」が完全に満たされた状態の自分を、五感(視覚/聴覚/感覚)で描写してください。",
    placeholder: "例: 朝起きて窓から光が差して..."
  },
  {
    id: "q4",
    title: "妨げているもの",
    q: "「将来不安シミュレーション」を妨げている、生活パターン・思考パターンは何ですか？",
    placeholder: "例: 寝る前のスマホ、朝のバタバタ..."
  },
  {
    id: "q5",
    title: "今夜の儀式",
    q: "今夜・明日できる、「将来不安シミュレーション」に向けた小さな儀式は？",
    placeholder: "例: 寝る前に1日3つ感謝をメモする..."
  }
];

const HISTORY_KEY = "app088_history_v1";
...
const buildAnalysisPrompt = (userName, answers) => {
  ...
  return `あなたは「将来不安シミュレーションの専門家」です。以下のユーザーの回答から、不安の正体を具体化し対策ための深い分析を提供してください。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "14px 18px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: C.gold }}>将来不安シミュレーション</div>
            <div style={{ fontSize: 10, color: C.textMuted }}>不安の正体を具体化し対策</div>
          </div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "28px 18px 40px" }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 12, lineHeight: 1.5 }}>将来不安シミュレーション</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>不安の正体を具体化し対策。あなたの中に眠る答えを引き出します。</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "18px 18px 40px" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={async () => {
              const text = `【将来不安シミュレーション】${selectedHistory.date}\n\n${selectedHistory.analysis}`;
