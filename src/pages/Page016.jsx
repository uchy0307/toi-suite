...
const QUESTIONS = [
  {
    id: "q1",
    title: "最近の怒り",
    q: "最近、怒りや強いフラストレーションを感じた出来事を3つ教えてください。\nできる限り具体的に。",
    placeholder: "例：1.〇〇の時に怒りを感じた 2.〇〇 3.〇〇..."
  },
  {
    id: "q2",
    title: "怒りの裏にあるもの",
    q: "それぞれの怒りの裏に、あなたが「大切にしているもの」「守りたいもの」は何だと思いますか？",
    placeholder: "例：〇〇への怒りの裏には、〇〇を大切にしているという思いがある..."
  },
  {
    id: "q3",
    title: "体の感覚",
    q: "怒りを感じた時、体のどこに、どんな感覚が生じますか？",
    placeholder: "例：怒りを感じると〇〇（体の部位）が〇〇になる感じがする..."
  },
  {
    id: "q4",
    title: "怒りの後の行動",
    q: "怒りを感じた後、どんな行動をとる傾向がありますか？",
    placeholder: "例：怒ると〇〇してしまう。その後〇〇と後悔することがある..."
  },
  {
    id: "q5",
    title: "怒らない人との違い",
    q: "同じ状況で怒らない人がいるとしたら、何が違うと思いますか？",
    placeholder: "例：〇〇が違うと思う..."
  }
];

const HISTORY_KEY = "app16_history_v1";
...
const buildAnalysisPrompt = (userName, answers) => {
  ...
  return `あなたは「怒りのトリガー分析の専門家」です。以下のユーザーの回答を踏まえ、深く分析してください。

あなたは「感情・怒り管理の専門家」です。

### 【あなたの怒りのトリガーマップ】
怒りが生じるパターンを整理し共通のトリガーを特定する。

### 【怒りの裏にある価値観】
各怒りの奥にある大切にしているものを明確にする。

### 【怒りのエネルギーの建設的活用】
怒りのエネルギーをどう行動・変革・創造に変換できるか提案する。

### 【怒りの管理テクニック】
この人に合った具体的な怒り管理の方法を3つ提案する。

### 【今週の実験】
1つの怒りトリガーを意識的に観察する実験を提案する。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "14px 18px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: C.gold }}>怒りのトリガー分析</div>
            <div style={{ fontSize: 10, color: C.textMuted }}>怒りの正体を知り、感情に振り回されない自分になる</div>
          </div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "28px 18px 40px" }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 12, lineHeight: 1.5 }}>怒りのトリガー分析</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>怒りの正体を知り、感情に振り回されない自分になる。あなたの中に眠る答えを引き出します。</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "18px 18px 40px" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={async () => {
              const text = `【怒りのトリガー分析】${selectedHistory.date}\n\n${selectedHistory.analysis}`;
