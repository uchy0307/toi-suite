...
const QUESTIONS = [
  {
    id: "q1",
    title: "心が動く瞬間",
    q: "涙が出るほど感動した経験、最近で構いません。何にそれほど反応しましたか?",
    placeholder: "例：子どもの成長を見た時、誰かが自分のために動いてくれたのを知った時..."
  },
  {
    id: "q2",
    title: "許せないこと",
    q: "自分が絶対に妥協できない・許せないことは何ですか?",
    placeholder: "例：不正直さ、人の努力を無視すること、選択肢を奪うこと..."
  },
  {
    id: "q3",
    title: "お金より大事なもの",
    q: "お金で買えないが、自分の人生で最も大事と感じることは?",
    placeholder: "例：自由、信頼、家族との時間、自分のペース、成長..."
  },
  {
    id: "q4",
    title: "過去の選択",
    q: "重要な決断で「なぜ私はこちらを選んだのか」を最近振り返ったことは?",
    placeholder: "例：〇〇という道を選んだのは、〇〇を大事にしたいから..."
  },
  {
    id: "q5",
    title: "理想の死に方",
    q: "50年後の自分が「これがあったから良い人生だった」と言える要素は?",
    placeholder: "例：意味のある何かを遺した、好きな人たちに囲まれていた..."
  }
];

const HISTORY_KEY = "app02_history_v1";
...
const buildAnalysisPrompt = (userName, answers, mode = "deep") => {
  ...
  return `あなたは価値観発掘コンサルタントです。以下の回答から、本人が無意識に持っている価値観を5つ抽出し、それぞれに対する根拠と、人生における意思決定への活用シーンを提案してください。

### 【抽出される5つの価値観】
各価値観に対して:
- 何という価値観か(一言で)
- その人の回答のどこから読み取ったか(根拠)
- その価値観に気づくことで人生のどんな決断が楽になるか(3〜4文)

### 【意思決定への活用シーン】
5つの価値観を踏まえたとき:
- 迷った時の判断基準として何を使えるか
- 現在地から見て「本当はやりたかったけど後回しにしていたこと」は何か

### 【あなたの人生軸】
5つの価値観を言語化した「あなただけの人生軸」を50字程度で描写してください。
...
const buildPerspectivePrompt = (userName, analysisText) => {
  return `あなたは「価値観と人生の専門家」です。以下の人物分析を踏まえ、3つの異なる視点からその人を見たときに何が見えるかを描写してください。

### 👫 親友・友人の視点
${userName ? userName + "さんを" : "この人を"}よく知る親友・友人として、「この人の本当の大事にしていること」「それに気づいているか」「それを活かしたら何が変わるか」を率直に語ってください。愛情ある正直さで。2〜3段落。

### ❤️ 大切な家族・パートナーの視点
身近にいる大切な人(パートナー・親・子など)として、「この人の判断基準で感じていること」「もっとこの人の価値観を大事にしてほしいこと」「一緒にいて感じる本質」を語ってください。2〜3段落。

### 🔮 未来の自分の視点(10年後)
10年後の${userName ? userName + "さん" : "あなた"}が今を振り返って語ってください。「あの頃、私は〇〇を大事にしていたが…」「その選択があったから…」「変わったこと・変わらないで良かったこと」。2〜3段落。
...
const buildDeepDivePrompt = (userName, analysisText, theme) => {
  return `あなたは「価値観発掘の専門家」です。以下の人物の分析結果を踏まえ、特定のテーマについて深掘りした追加分析を行ってください。

### 深掘りテーマ
${theme}

### 出力フォーマット
1. このテーマがその人の価値観とどう繋がっているか(3〜4文)
2. 具体的な行動・選択を3パターン(「〇〇する」という形式で)
3. よくある落とし穴を1つ(そしてその対策も)
4. 今日から始められる小さな実験を1つ
...
const buildHistorySummaryPrompt = (history) => {
  ...
  return `あなたは継続的な価値観の成熟をサポートするコーチです。以下は同じ人の過去${recent.length}回分のセッション履歴です。

この人の価値観の深化・成長・繰り返されているテーマ・今後の課題を200〜300字程度でサマリしてください。
過去から現在への流れが分かるように、時系列を意識して、「最初は〜だったが、今は…」という成長の軌跡を書いてください。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "14px 18px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          <div style={{ width: 40, height: 40, borderRadius: "50%", background: `linear-gradient(135deg,${C.gold},${C.goldDim})`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 19 }}>⚖️</div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: C.gold }}>価値観発掘コンサル</div>
            <div style={{ fontSize: 10, color: C.textMuted }}>人生の意思決定の軸を見つける</div>
          </div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "28px 18px 40px" }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 12, lineHeight: 1.5 }}>価値観発掘コンサル</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>あなたの根本的な価値観を言語化し、人生の意思決定の軸を作ります。迷った時の判断基準となる「本当の大事にしていること」を見つけましょう。</div>
          </div>
          <div style={{ background: C.goldBg, border: `1px solid ${C.borderActive}`, borderRadius: 12, padding: 14, marginBottom: 20, fontSize: 11.5, color: C.gold, lineHeight: 1.7 }}>
            ...
            このアプリは「価値観発掘プロンプト」を生成します。それを ChatGPT / Claude / Gemini にコピーして使えば、料金もAPI契約も不要。あなた自身のAIが分析してくれます。
          </div>
      ...
      {screen === "result" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 18px 40px" }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: C.gold, marginBottom: 4 }}>✨ 価値観分析プロンプトが生成されました</div>
          ...
          <PromptCard title="📋 価値観分析プロンプト" prompt={analysisPrompt} />
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            {perspPrompt && <PromptCard title="👥 多視点分析プロンプト" prompt={perspPrompt} />}
          </div>

          {/* 深掘り */}
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: 14, marginBottom: 14 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: C.textSub, marginBottom: 8 }}>🔍 特定テーマで深掘り</div>
            <input value={deepTheme} onChange={e => setDeepTheme(e.target.value)} placeholder="例：仕事での活かし方 / 家族関係への活かし方..." style={{ width: "100%", background: C.surface2, border: `1px solid ${C.border}`, borderRadius: 9, color: C.text, padding: "9px 12px", fontSize: 12, marginBottom: 8 }} />
              ...
              {history.length >= 2 && (
                <div style={{ marginBottom: 14 }}>
                  <button onClick={generateHistorySummary} style={{ width: "100%", padding: "10px 0", marginBottom: 8, background: C.goldBg, border: `1px solid ${C.borderActive}`, borderRadius: 10, color: C.gold, fontSize: 12, fontWeight: 600 }}>
                    🧠 過去{history.length}回分の価値観成長プロンプトを生成
                  </button>
                  {summaryPrompt && <PromptCard title="🧠 成長分析プロンプト" prompt={summaryPrompt} />}
                </div>
              )}
