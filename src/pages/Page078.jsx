...
const QUESTIONS = [
  {
    id: "q1",
    title: "動機",
    q: "「習慣化ハードル下げ」を伸ばしたい・身につけたい本当の理由は何ですか？",
    placeholder: "例: 仕事で〇〇したいから、子供に〇〇を見せたいから..."
  },
  {
    id: "q2",
    title: "過去の試行",
    q: "これまで「習慣化ハードル下げ」について試したこと、その結果はどうでしたか？",
    placeholder: "例: 〇〇という本を読んだ、〇〇講座を受けた..."
  },
  {
    id: "q3",
    title: "マスター後の自分",
    q: "「習慣化ハードル下げ」を完全にマスターした自分は、何ができていますか？",
    placeholder: "例: 1時間で資料が完成する、抽象化が瞬時にできる..."
  },
  {
    id: "q4",
    title: "学習の壁",
    q: "「習慣化ハードル下げ」を学ぶ上で、つい挫折してしまうポイントはどこですか？",
    placeholder: "例: 続かない、難しさで折れる、優先順位が下がる..."
  },
  {
    id: "q5",
    title: "最小の練習",
    q: "今週から始められる、「習慣化ハードル下げ」を伸ばす最小の練習は？",
    placeholder: "例: 毎朝5分、〇〇する..."
  }
];

const HISTORY_KEY = "app078_history_v1";
...
const buildAnalysisPrompt = (userName, answers) => {
  ...
  return `あなたは「習慣化ハードル下げの専門家」です。以下のユーザーの回答から、「これならできる」最小単位ための深い分析を提供してください。
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "14px 18px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: C.gold }}>習慣化ハードル下げ</div>
            <div style={{ fontSize: 10, color: C.textMuted }}>「これならできる」最小単位</div>
          </div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "28px 18px 40px" }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 12, lineHeight: 1.5 }}>習慣化ハードル下げ</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>「これならできる」最小単位。あなたの中に眠る答えを引き出します。</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "18px 18px 40px" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={async () => {
              const text = `【習慣化ハードル下げ】${selectedHistory.date}\n\n${selectedHistory.analysis}`;
