...
const RALLY_QUESTIONS = [
  { id: "q1", step: 1, q: "今、頭の中にある「未解決のこと」「気になっていること」を全部書き出してください。", placeholder: "例：〇〇のことが気がかり、〇〇との関係がモヤモヤする、寝る前に思い出すこと... 小さなことでも全部" },
  { id: "q2", step: 2, q: "書き出したものの中で、今「一番心が重い」と感じているのはどれですか？なぜ重いのですか？", placeholder: "例：〇〇が一番重い。なぜなら..." },
  { id: "q3", step: 3, q: "それはいつから？きっかけになった出来事はありますか？", placeholder: "例：〇月から / 〇〇があった日から / きっかけは思い当たらない など" },
  { id: "q4", step: 4, q: "これまでにどう対処してきましたか？効いた・効かなかったのはどんな対処でしたか？", placeholder: "例：友人に話して少し楽になった / 我慢していたが悪化した / ネットで調べたが解決しなかった など" },
  { id: "q5", step: 5, q: "もし明日、このモヤモヤが消えていたら、何が一番変わると思いますか？", placeholder: "例：朝起きるのが楽になる / 気になっていた人に連絡できる / 仕事に集中できる など" },
  { id: "q6", step: 6, q: "そのモヤモヤのうち、自分でコントロールできる部分はどこですか？できない部分は？", placeholder: "コントロールできる：...\nコントロールできない：..." },
  { id: "q7", step: 7, q: "親友が同じ悩みを抱えていたら、あなたは何と声をかけますか？", placeholder: "例：「そんな自分を責めなくていいよ」「一回休んでみたら？」など、親友にかける優しい言葉" },
  { id: "q8", step: 8, q: "最悪の場合、何が起きますか？それが現実になる確率は何%くらいだと思いますか？", placeholder: "最悪の場合：...\n確率：約〇%" },
  { id: "q9", step: 9, q: "1年後、このモヤモヤはどうなっていそうですか？残っているとしたら何が変わっていますか？", placeholder: "例：解決している / 形を変えて残っている / もっと悪化している など" },
  { id: "q10", step: 10, q: "今夜寝る前にできる、ほんの小さな一歩は何ですか？", placeholder: "例：3分だけ深呼吸 / 信頼できる人にLINE / お風呂にゆっくり浸かる など" },
];
...
const HEALTH_AXES = [
  { id: "sleep",    icon: "🌙", label: "睡眠",   desc: "寝つき・眠りの深さ・朝の目覚め" },
  { id: "appetite", icon: "🍴", label: "食欲",   desc: "食事量・味覚・体調の安定" },
  { id: "focus",    icon: "🎯", label: "集中",   desc: "仕事や物事に集中できる感覚" },
  { id: "mood",     icon: "💭", label: "気分",   desc: "感情の安定・落ち込みの少なさ" },
  { id: "social",   icon: "👥", label: "対人",   desc: "人と接する余裕・人付き合いの楽しさ" },
  { id: "joy",      icon: "✨", label: "興味",   desc: "好きなこと・楽しいことへの関心" },
];
...
時間スコープ: ${(persona.time || []).join("・") || "(未選択)"}`;
  ...
  return `あなたは「メンタル・デトックスの専門家」です。認知行動療法(CBT)・成功循環モデル・マズロー欲求段階の知見を踏まえ、以下の人物のモヤモヤを構造化し、解放への道筋を提示してください。
...
### 【本当の原因の特定】
表面的なモヤモヤの奥にある根本原因を、ペルソナ4軸と10ラリー回答を統合して論理的に示してください（300〜400字）。
...
const buildPerspectivePrompt = (userName, analysisText) => {
  return `あなたは「他者視点分析の専門家」です。以下のメンタル・デトックス分析を踏まえ、3つの異なる視点からその人を見たときに何が見えるかを描写してください。
...
const buildDeepDivePrompt = (userName, analysisText, theme) => {
  return `あなたは「強み言語化の専門家」です。以下の人物のメンタル・デトックス分析結果を踏まえ、特定のテーマについて深掘りした追加分析を行ってください。
...
export default function App() {
  ...
  const [radarScores, setRadarScores] = useState({
    sleep: 3, appetite: 3, focus: 3, mood: 3, social: 3, joy: 3,
  });
  ...
  const loadFromHistory = (rec) => {
    ...
    setRadarScores(rec.radarScores || { sleep: 3, appetite: 3, focus: 3, mood: 3, social: 3, joy: 3 });
    ...
    setAnalysisPrompt(buildAnalysisPrompt(
      ...
      rec.radarScores || { sleep: 3, appetite: 3, focus: 3, mood: 3, social: 3, joy: 3 }
    ));
  ...
  const resetAll = () => {
    ...
    setRadarScores({ sleep: 3, appetite: 3, focus: 3, mood: 3, social: 3, joy: 3 });
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "sans-serif", maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "12px 16px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, boxShadow: "0 1px 6px rgba(0,0,0,0.06)", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
          <div style={{ width: 36, height: 36, borderRadius: "50%", background: `linear-gradient(135deg,${C.gold},${C.goldDim})`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>🌫️</div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: C.goldDim }}>メンタル・デトックス</div>
            <div style={{ fontSize: 9, color: C.textMuted }}>4階層カテゴリ × 10ラリー × 健康レーダー</div>
          </div>
      ...
      {screen === "home" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "24px 18px 40px", animation: "fadeUp 0.4s ease" }}>
          <div style={{ textAlign: "center", marginBottom: 24 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: C.gold, marginBottom: 8, lineHeight: 1.5 }}>メンタル・デトックス v2</div>
            <div style={{ fontSize: 13, color: C.textSub, lineHeight: 1.9 }}>心のモヤモヤを<span style={{ color: C.goldDim, fontWeight: 600 }}>4階層カテゴリ・10問ラリー・6軸レーダー</span>で構造化し、感情を解放する。CBT × 成功循環 × マズロー。</div>
          </div>
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: 18, marginBottom: 20 }}>
            {[
              ...
              { n: "02", t: "メンタル健康レーダー", d: "睡眠・食欲・集中・気分・対人・興味の6軸を1〜5で評価" },
      ...
      {screen === "persona" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 32px" }}>
          ...
          <button onClick={personaReady ? goToRadar : null} disabled={!personaReady}
            ...
            {personaReady ? "▶ メンタル健康レーダーへ" : "まずは1項目以上選択"}
          </button>
      ...
      {screen === "result" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 32px" }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: C.goldDim, marginBottom: 12 }}>📋 メンタル・デトックス分析</div>
